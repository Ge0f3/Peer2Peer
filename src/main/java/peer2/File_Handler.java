...
public class File_Handler {
    ...
    public void Register_a_File(String fileName)                      //Register with index server Method
    {
        String message;
        /////////////////////////////////////////////////////////////////////////////
        System.out.printf(" 1 Registering file  %s \n",(fileName));
        if (file_exist(fileName))                                     //check if the file exist on the working directory
        {
            System.out.printf("Registering file %s \n",(fileName));
            try {
                ...
                out.flush();
                message = peerID + "-" + fileName;                        // append the file name and peer ID
                out.writeObject(message);                                 //send the message
                out.flush();
                /////////////////////////////////////////////////////////////////////////////
                System.out.println("File '"+fileName+"' index has been added successfully on the server!!\n");
        ...
        else                                                           //file is not on the working directory
        {
            System.out.println("File not found on your computer !!\n");
        }
    }
    ...
    boolean file_exist(String fileName) {                              //check the file existence on the working directory
        final File f = new File(peer2.class.getProtectionDomain().getCodeSource().getLocation().getPath()); //get the jar directory
        File parentFolder = new File(f.getParent());                                     //get the parent folder of the jar
        File dir = new File(parentFolder.getParent());
        File folder = new File(dir.getParent()+"/Peer2Peer/src/main/java/peer2/Files"); //get the resources folder path
    ...
    public void Create_Local_File(String fileName,String fileContent) //write the downloaded file into the local director
    {
        try
        {
            ...
            File parentFolder = new File(f.getParent());                                     //get the parent folder of the jar
            File dir = new File(parentFolder.getParent());
            File folder = new File(dir.getParent()+"/Peer2Peer/src/main/java/peer2/Files"); //get the resources folder pathath
