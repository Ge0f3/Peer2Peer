...
public class peer2 extends Thread{
    ...
    public static void main(String[] args) throws InterruptedException {
        ...
        /////////////////////////////////////////////////////////////////////////////
        String peerID = "60003";             // the peer ID used as a port listener as well
        ...
        try
        {
            thread = new Thread (new peer2(Integer.parseInt(peerID))); //initiate listener thread
        ...
        while (true)
        {
            ...
            else if (userInput.equals("6"))                    //if user entered 6
            {
                ...
                if(fh.isValidName(userInput)) {
                    ...
                    if(in.hasNextInt()){
                        loop = in.nextInt();
                        System.out.println(loop+" search requests  rate is " + get_performance_measurement_for_search_request(fh, userInput.trim(),loop) + " ms.");
                    }else{
    ...
    public static void list_my_files()
    {
        try {
            ...
            System.out.printf("Peer 2 Files are in %s \n",(folder));
            File[] listOfFiles = folder.listFiles();                                        //store files into file array
            System.out.printf(" No of files in peer 2 %s \n",(listOfFiles.length));
    ...
    public static void register_all_files(File_Handler fh) {
        try {
            ...
            File dir = new File(parentFolder.getParent());
            File folder = new File(dir.getParent()+"/Peer2Peer/src/main/java/peer2/Files"); //get the resources folder path
            System.out.printf("Peer 1 Files are in %s \n",(folder));
            File[] listOfFiles = folder.listFiles();                                        //store files into file array
            System.out.printf(" No of files in peer 2 %s \n",(listOfFiles.length));
            if (!listOfFiles.equals(null)) {                                                //if folder is not empty
                for (int i = 0; i < listOfFiles.length; i++) {
                    if (listOfFiles[i].isFile() && listOfFiles[i].getName().contains("txt")) //make sure it is a file and .txt
                        System.out.printf("Tryin to register %s \n",(listOfFiles[i].getName()));
